...
def find_necklace_width(mask):
    ...
    print("First non-zero pixel coordinates (x, y):", first_non_zero)
    ...
    print("second non-zero pixel coordinates (x, y):", second_non_zero)

    return second_non_zero[0]-first_non_zero[0]
...
def apply_necklace_to_model(image, segmented_necklace, necklace_mask):
    """
    Apply pre-segmented necklace to a model image.
    This replaces the original detect_neck_and_adjust_necklace function.

    Args:
        image: The model image to apply the necklace to
        segmented_necklace: The pre-segmented necklace image
        necklace_mask: The necklace mask from segmentation
    """
    import numpy as np
    import cv2
    from mediapipe.python.solutions.face_mesh import FaceMesh
    ...
    # Calculate necklace width from the mask - this is specific to each necklace
    #necklace_width = find_necklace_width(necklace_mask)

    necklace_point1, necklace_point2, necklace_point3 = find_necklace_3point(necklace_mask)
    necklace_width = (necklace_point2[0] - necklace_point1[0])
    segmented_necklace = segmented_necklace[necklace_point1[1]:necklace_point3[1], ]
    ...
    if result.multi_face_landmarks:
        ...
        chin_index = 152  # Chin tip index
        left_jaw_index = 234  # Left jaw index
        right_jaw_index = 454  # Right jaw index
        ...
        ratio = (neck_width / necklace_width) * 0.85  # fine neck hyperparameters-0.95?
        necklace_resized = cv2.resize(segmented_necklace,
                                      (int(segmented_necklace.shape[1] * ratio),
                                       int(segmented_necklace.shape[0] * ratio)))

        # Resize the necklace based on the neck width of THIS model
        #if necklace_width <= 0 or necklace_width == 1:
         #   necklace_resized = cv2.resize(segmented_necklace,
          #                                (neck_width + 15,
           #                                int(neck_width * segmented_necklace.shape[0] / segmented_necklace.shape[1])))
        #else:
         #   ratio = (neck_width / necklace_width) * 0.80  # fine neck hyperparameters-0.95?
          #  necklace_resized = cv2.resize(segmented_necklace,
           #                               (int(segmented_necklace.shape[1] * ratio),
            #                               int(segmented_necklace.shape[0] * ratio)))
...
def apply_necklace_to_model_manual(image, segmented_necklace, necklace_mask):
    """
    Apply pre-segmented necklace to a model image with manual positioning.
    This replaces the original detect_neck_and_adjust_necklaces function for manual mode.
    ...
    Args:
        ...
        necklace_mask: The necklace mask from segmentation
    """
    # This would be your implementation for manual placement
    # For now, I'm providing a simplified version that would need to be expanded
    # based on how your manual placement UI works
    ...
    # Calculate necklace width from the mask
    #necklace_width = find_necklace_width(necklace_mask)
    ...
    if result.multi_face_landmarks:
        # Similar to automatic mode, but would incorporate manual positioning
        # You would need to adapt this based on how your manual placement works

        # For now, this is just a placeholder that works like the automatic mode
        landmarks = result.multi_face_landmarks[0].landmark
        ...
        # Resize the necklace based on the neck width
        #if necklace_width <= 0 or necklace_width == 1:
         #   necklace_resized = cv2.resize(segmented_necklace,
          #                                (neck_width + 15,
           #                                int(neck_width * segmented_necklace.shape[0] / segmented_necklace.shape[1])))
        #else:
         #   ratio = (neck_width / necklace_width) * 0.80
          #  necklace_resized = cv2.resize(segmented_necklace,
           #                               (int(segmented_necklace.shape[1] * ratio),
            #                               int(segmented_necklace.shape[0] * ratio)))

        # In manual mode, you might have UI elements for x_offset and y_offset adjustments
        x_offset = chin_x - necklace_resized.shape[1] // 2
...
def main():
    ...
    # UI mode selection: Single image or Batch processing
    processing_mode = st.radio("Processing Mode:", ('Single Image', 'Batch Processing'))
    ...
    if "output_folder" not in st.session_state:
        st.session_state.output_folder = "output_images"

    # Fine-Tune Makeup Section in sidebar (for both modes)
    ...
    concealer_n = 1
    if concealer:
        status = st.sidebar.radio("Select Place to apply Concealer:", ('Chin', 'Nose', 'Under Eye'))
        if status == 'Chin':
            concealer_n = 1
        if status == 'Nose':
            concealer_n = 2
    ...
    if processing_mode == 'Single Image':
        ...
        model_image_file = st.file_uploader("Upload Model Image", type=["jpg", "jpeg", "png"],
                                            key="single_model_upload")

        if model_image_file:
            file_bytes = model_image_file.getvalue()  # Get bytes directly
            nparr = np.frombuffer(file_bytes, np.uint8)
            st.session_state.model_image = cv2.imdecode(nparr, cv2.IMREAD_COLOR)
        ...
        necklace_image_file = st.file_uploader("Upload Necklace Image (PNG with transparency)", type=["jpg", "jpeg", "png"],
                                               key="single_necklace_upload")

        if necklace_image_file and st.session_state.model_image is not None:
            if st.session_state.necklace_image != necklace_image_file:
                st.session_state.necklace_image = necklace_image_file
                try:
                    # Pass the file object directly - don't decode it here
                    necklace_file = necklace_image_file

                    if mode == "Manual":
                        st.session_state.jewelry_output_image = detect_neck_and_adjust_necklaces(
                            st.session_state.model_image, necklace_file
                        )
                    else:  # Auto mode
        ...
        # Apply fine-tune makeup (Single mode)
        if st.button("Apply Fine-Tune Makeup"):
            if st.session_state.jewelry_output_image is not None and st.session_state.transferred_image is not None:
        ...
        col1, col2 = st.columns(2)
        with col1:
            st.session_state.output_folder = st.text_input("Output Folder", "output_images")
        with col2:
            necklace_mode = st.selectbox("Necklace Fitting Mode", ["Auto", "Manual"], index=0)

        # 1. Upload folder of images
        st.subheader("1. Upload Model Images")
        batch_image_files = st.file_uploader("Upload multiple model images", type=["jpg", "jpeg", "png"],
                                             accept_multiple_files=True, key="batch_model_upload")

        if batch_image_files:
            # Clear previous batch images when new ones are uploaded
            st.session_state.batch_images = []

            for file in batch_image_files:
                try:
                    # Get bytes directly from file object
            ...
            # Display preview of batch images
            if len(st.session_state.batch_images) > 0:
                st.markdown(f"**Successfully loaded {len(st.session_state.batch_images)} images**")
                preview_count = min(6, len(st.session_state.batch_images))
                cols = st.columns(3)
                for i in range(preview_count):
                    with cols[i % 3]:
                        st.image(st.session_state.batch_images[i]["image"],
                                 caption=st.session_state.batch_images[i]["name"],
                                 channels="BGR",
                                 width=120)

        # 2. Upload necklace image
        st.subheader("2. Upload Necklace Image")
        batch_necklace_file = st.file_uploader("Upload Necklace Image (PNG with transparency)",
                                               type=["jpg", "jpeg", "png"],
                                               key="batch_necklace_upload")

        # 3. Upload reference makeup image (optional)
        st.subheader("3. Upload Reference Makeup Image (Optional)")
        apply_makeup = st.checkbox("Apply makeup to processed images", value=False)
        reference_image = None

        if apply_makeup:
            batch_reference_file = st.file_uploader("Upload Reference Image for Makeup", type=["jpg", "jpeg", "png"],
                                                    key="batch_reference_upload")
            if batch_reference_file:
                try:
                    reference_image = Image.open(batch_reference_file).convert("RGB")
                    st.image(reference_image, caption="Makeup Reference Image", width=200)
                except Exception as e:
                    st.error(f"Error loading reference image: {str(e)}")

        # 4. Process all images
        if st.button("Process All Images") and st.session_state.batch_images and batch_necklace_file is not None:
            ...
            # Process necklace image once - do the segmentation just once
            try:
                # Read necklace image data
                necklace_bytes = np.frombuffer(batch_necklace_file.getvalue(), np.uint8)
                necklace_img = cv2.imdecode(necklace_bytes, cv2.IMREAD_UNCHANGED)

                # Pre-segment the necklace once
                original_necklace, necklace_mask, segmented_necklace = segment_jewellery(necklace_img)

                # Find width once (though this might be recalculated per model if needed)
                base_necklace_width = find_necklace_width(necklace_mask)

                st.success("Necklace image processed successfully")

            except Exception as e:
                st.error(f"Error processing necklace image: {str(e)}")
                st.stop()

            # Process each image
            for i, img_data in enumerate(st.session_state.batch_images):
                try:
                    status_text.text(
                        f"Processing image {i + 1}/{len(st.session_state.batch_images)}: {img_data['name']}")

                    # 1. Apply necklace using pre-segmented necklace
                    model_image = img_data["image"]

                    if necklace_mode == "Auto":
                        # Modified function that takes pre-segmented necklace
                        jewelry_result = apply_necklace_to_model(model_image, segmented_necklace, necklace_mask)
                    else:  # Manual mode - you would need to create a corresponding function for manual mode
                        jewelry_result = apply_necklace_to_model_manual(model_image, segmented_necklace, necklace_mask)

                    final_result = jewelry_result  # Default if no makeup applied

                    # 2. Apply makeup if reference provided
                    if apply_makeup and reference_image is not None:
                        # Convert to RGB for makeup transfer
                        jewelry_rgb = cv2.cvtColor(jewelry_result, cv2.COLOR_BGR2RGB)
                        source_image = Image.fromarray(jewelry_rgb).convert("RGB")

                        # Transfer makeup - both source_image and reference_image are PIL Images
                        transferred = transfer_makeup(source_image, reference_image)

                        # 3. Apply fine-tune makeup if checked
                        if any([lipstick, saturation, concealer, undereye, blush, lipliner,
                                eyebrows, toner, eyeshadow]):
                            results = face_mesh.process(cv2.cvtColor(jewelry_result, cv2.COLOR_BGR2RGB))
                            final_result = apply_fine_tune_makeup(
                                transferred, results,
                                lipstick, lipstick_color, undereye, eye_liner_color,
                                concealer, concealer_color, concealer_n, blush, blush_color,
                                toner, toner_color, eyebrows, lipliner, lipliner_color, eyebrow_color,
                            ...
                            if isinstance(transferred, Image.Image):
                                final_result = np.array(transferred)
                            else:
                                final_result = transferred

                    # Save the result
                    output_path = os.path.join(st.session_state.output_folder, f"processed_{img_data['name']}")

                    # Make sure final_result is a numpy array before saving
                    if not isinstance(final_result, np.ndarray):
                        final_result = np.array(final_result)

                    # Ensure correct format for OpenCV imwrite (BGR for color images)
                    if len(final_result.shape) == 3 and final_result.shape[2] == 3:
                        # Check if the image is in RGB format (from PIL) and convert to BGR if needed
                        if isinstance(final_result, np.ndarray) and final_result.shape[2] == 3:
                            # If it came from a PIL Image, it's likely in RGB format
                            if apply_makeup and reference_image is not None:
            ...
            if len(st.session_state.processed_images) > 0:
                status_text.text(f"Processing complete! {len(st.session_state.processed_images)} images processed.")

                st.subheader("Results")
                st.success(f"All processed images saved to folder: `{st.session_state.output_folder}`")

                # Display preview of processed images
                preview_count = min(6, len(st.session_state.processed_images))
                st.markdown("##### Preview of processed images")

                cols = st.columns(3)
                for i in range(preview_count):
                    with cols[i % 3]:
                        # Ensure proper color channel ordering for display
                        display_img = st.session_state.processed_images[i]["image"]
                        if len(display_img.shape) == 3 and display_img.shape[2] == 3:
                            # Images saved with cv2 are in BGR format, convert to RGB for display
                            st.image(display_img,
                                     caption=st.session_state.processed_images[i]["name"],
                                     channels="BGR",  # Specify BGR so Streamlit knows to convert
                                     width=150)
                        else:
                            st.image(display_img,
                                     caption=st.session_state.processed_images[i]["name"],
                ...
                if st.button("Download All Processed Images as ZIP"):
                    try:
                        # Create a ZIP file containing all processed images
                        zip_path = os.path.join(st.session_state.output_folder, "processed_images.zip")
                        with zipfile.ZipFile(zip_path, 'w') as zipf:
                            for img_data in st.session_state.processed_images:
                                if os.path.exists(img_data["path"]):
                                    zipf.write(img_data["path"], os.path.basename(img_data["path"]))
                ...
                status_text.text("Processing complete, but no images were successfully processed.")
